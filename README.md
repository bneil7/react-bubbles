...
- [x] Explain what a token is used for.
      // Tokens are assigned to a user with proper authentication credentials to allow them to access the data on a secure resource server.

- [x] What steps can you take in your web apps to keep your data secure?
      // Use complex encryption and dynamic authentication processes in your code.

- [x] Describe how web servers work.
      // Web servers store data to serve/communicate to other computers around the world via the web (with proper credentials, of course).
      1, the client makes an auth request to the auth server
      2, if the client has proper credentials, the auth server assigns them an access token
      3, the client uses that access token to gain permission to data on a separate resource server
      4, the resource server sends the authorized client the protected data once they are trusted.
